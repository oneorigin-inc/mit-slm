...
async def call_model_async(prompt: str, config: dict = None, stream: bool = False) -> Union[str, AsyncGenerator[str, None]]:
    """
    Call Ollama model with optional streaming support
    ...
    Returns:
        String response (if stream=False) or AsyncGenerator (if stream=True)
    """
    if config is None:
        config = MODEL_CONFIG
    ...
    try:
        ...
        if stream:
            async def error_generator():
                yield "Error: Model request timed out"
        ...
        if stream:
            async def error_generator():
                yield f"Error: Model API error: {e}"
            return error_generator()
        else:
...
async def _stream_ollama_response(payload: dict) -> AsyncGenerator[str, None]:
    """Internal function to handle streaming response from Ollama"""
    try:
        async with httpx.AsyncClient(timeout=httpx.Timeout(120.0)) as client:
...
def calculate_font_size(text: str, base_size: int) -> int:
    ...
    # Scale down for longer text, stay within 36-45 range (10% smaller)
    if len(text) > 30:
...
def generate_badge_config(
    ...
    for idx, txt in enumerate(texts):
        ...
        # Font size within spec (36-45) - 10% smaller
...
async def generate_badge_metadata_async(request, stream: bool = False) -> Union[dict, AsyncGenerator[dict, None]]:
    """
    Generate badge metadata with optional streaming support
    
    Args:
        request: Badge request object
        stream: Whether to stream the generation process
    
    Returns:
        Complete metadata dict (if stream=False) or AsyncGenerator yielding progress (if stream=True)
    """
...
- Criterion Style: {CRITERION_TEMPLATES.get(random_params['criterion_style'])}"""
    ...
    if stream:
        ...
        return _generate_badge_metadata_streaming(user_content, random_params, processed_course_input)
    else:
        # Return complete result
        response = await call_model_async(user_content, stream=False)
...
async def _generate_badge_metadata_streaming(user_content: str, random_params: dict, processed_course_input: str) -> AsyncGenerator[dict, None]:
    """Internal streaming generator for badge metadata"""
    full_response = ""
    ...
    yield {
        ...
        "message": "Model is generating badge metadata...",
        "partial_response": ""
    }
    
    # Stream model response
    model_stream = await call_model_async(user_content, stream=True)
    ...
    try:
        ...
        result["raw_model_output"] = full_response
        result["selected_parameters"] = random_params
        result["processed_course_input"] = processed_course_input
        ...
        yield {
            ...
            "message": "Badge metadata generated successfully",
            "result": result
        }
    except Exception as e:
        yield {
            ...
            "message": f"Error processing model response: {str(e)}",
...
async def generate_badge(request: BadgeRequest):
    ...
    try:
        # Generate badge metadata with random parameters (non-streaming)
        badge_json = await generate_badge_metadata_async(request, stream=False)
...
async def generate_badge_streaming(request: BadgeRequest):
    ...
    async def badge_stream():
        start_time = time.time()
        
        try:
            # Send initial status
            yield f"data: {json.dumps({'status': 'starting', 'message': 'Initializing badge generation...'})}\n\n"
            
            # Stream metadata generation
            yield f"data: {json.dumps({'status': 'generating_metadata', 'message': 'Creating badge metadata...'})}\n\n"
            
            badge_json = None
            metadata_stream = await generate_badge_metadata_async(request, stream=True)
            
            async for update in metadata_stream:
                # Send each update to client
                yield f"data: {json.dumps(update)}\n\n"
                
                # Capture final result
                if update.get("type") == "completed":
                    badge_json = update.get("result")
            
            if not badge_json:
                yield f"data: {json.dumps({'type': 'error', 'message': 'Failed to generate metadata'})}\n\n"
                return
            
            # Continue with validation
            yield f"data: {json.dumps({'status': 'validating', 'message': 'Validating badge data...'})}\n\n"
            ...
            try:
                ...
                yield f"data: {json.dumps({'type': 'error', 'message': f'Validation error: {ve}'})}\n\n"
                return
            
            # Send metadata preview
            yield f"data: {json.dumps({'status': 'metadata_ready', 'message': f'Generated: {validated.badge_name}', 'data': {'name': validated.badge_name, 'description': validated.badge_description[:100] + '...' if len(validated.badge_description) > 100 else validated.badge_description}})}\n\n"
            
            # Generate image config
            image_type = random.choice(["text_overlay", "icon_based"])
            yield f"data: {json.dumps({'status': 'generating_image_config', 'message': f'Configuring {image_type} image...'})}\n\n"
            
            if image_type == "icon_based":
                yield f"data: {json.dumps({'status': 'finding_icons', 'message': 'Finding relevant icons...'})}\n\n"
                icon_suggestions = await get_icon_suggestions_for_badge(
                    badge_name=validated.badge_name,
                    badge_description=validated.badge_description,
                    custom_instructions=request.custom_instructions or "",
                ...
                image_config_wrapper = await generate_icon_image_config(
                    validated.badge_name, validated.badge_description, 
                    icon_suggestions, request.institution or ""
                )
            else:
                yield f"data: {json.dumps({'status': 'optimizing_text', 'message': 'Optimizing text layout...'})}\n\n"
                optimized_text = await optimize_badge_text({
                    "badge_name": validated.badge_name,
                    "badge_description": validated.badge_description,
                    "institution": request.institution or ""
                ...
                image_config_wrapper = await generate_text_image_config(
                    validated.badge_name, validated.badge_description,
                    optimized_text, request.institution or ""
                )
            
            image_config = image_config_wrapper.get("config", {})
            
            # Generate image
            yield f"data: {json.dumps({'status': 'generating_image', 'message': 'Creating badge image...'})}\n\n"
            image_base64 = await generate_badge_image(image_config)
            ...
            result = BadgeResponse(
                credentialSubject={
                    "achievement": {
                        ...
                        "description": validated.badge_description,
                        "image": {"id": f"https://example.com/achievements/badge_{badge_id}/image", "image_base64": image_base64},
                        "name": validated.badge_name
                    }
                },
                imageConfig=image_config,
            ...
            # Send final result
            yield f"data: {json.dumps({'status': 'completed', 'message': 'Badge generation completed!', 'result': result.dict(), 'generation_time': history_entry['generation_time']})}\n\n"
